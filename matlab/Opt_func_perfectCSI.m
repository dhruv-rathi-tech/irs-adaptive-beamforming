function [ Sum_Rate, P, Theta ] =  Opt_func_perfectCSI( M,N,K,P_max,sigma,H,G )
...
% syms  dual_v
power = @(dual_v) norm( pinv(dual_v*eye(N)+T + 1e-10*eye(N))*H_all'*mu, 'fro' )^2 - P_max;
low = 1e-6;
high = 50;
tolerance = 1e-5;
...
Theta_opt =  pinv(A + diag(zeta) + 1e-8*eye(M)) * b;
% 
